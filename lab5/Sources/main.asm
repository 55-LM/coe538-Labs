...
; equates section
;*******************************************************************
LCD_DAT           EQU PORTB                              ; LCD data port is connected to Port B
LCD_CNTR          EQU PTJ                                ; LCD control port is connected to Port J
LCD_E             EQU $80                                ; Set Enable signal (pin 7 on PTJ) on LCD
LCD_RS            EQU $40                                ; Set Register select (pin 6 of PTJ) on LCD
...
NOT_START         CMPA #FWD                              ; Compare value in ACCA to forward state   |
                  BNE NOT_FORWARD                        ; Branch if state is not equal to FWD      |
                  JSR FWD_ST                             ; If equal to FWD jump to FWD_ST           |
                  JMP DISP_EXIT                          ; Jump to exit from state dispatcher       |
                                                         ;                                          |
NOT_FORWARD       CMPA #REV                              ; Compare value in ACCA to reverse state   |
                  BNE NOT_REVERSE                        ; Branch if state is not equal to REV      D
                  JSR REV_ST                             ; If equal to FWD jump to REV_ST           I
                  JMP DISP_EXIT                          ;                                          S
                                                         ;                                          P
NOT_REVERSE       CMPA #ALL_STP                          ; Compare value in ACCA to All stop state  A
                  BNE NOT_ALL_STOP                       ;                                          T
                  JSR ALL_STP_ST                         ;                                          C
                  JMP DISP_EXIT                          ;                                          H
                                                         ;                                          E
...
;*******************************************************************
FWD_ST            BRSET PORTAD0,$04,NO_FWD_BUMP          ; If bumper is set (1) then branch to NO_FWD_BUMP
                  JSR INIT_REV                           ; Jump to INIT_REV to initilize reverse state
                  MOVB #REV,CRNT_STATE                   ; Set current state as REV
                  JMP FWD_EXIT                           ; 

NO_FWD_BUMP       BRSET PORTAD0,$08,NO_REAR_BUMP         ; If rear bumper is set then branch to NO_REAR_BUMP
                  JSR INIT_ALL_STP                       ; Jump to intilizing the All stop state
                  MOVB #ALL_STP,CRNT_STATE               ; Set current state as All stop
                  JMP FWD_EXIT                           ; 
...
NO_REAR_BUMP      LDAA TOF_COUNTER                       ; If Tc>Tfwd then
                  ...
                  MOVB #FWD_TRN,CRNT_STATE               ; and return
                  JMP FWD_EXIT
...
ISBLANK1          LDAA    #' '                           ; It’s blank (Could also use #$20)
